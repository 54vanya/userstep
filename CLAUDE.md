...
- **Ввод нот**: клик = tap (по ноте — удалить), drag вниз = hold (через границы блоков), Alt+drag = серия тапов; клавиши-колонки работают всегда — при playback live-запись (tap на ближайшую строку, замещение), на паузе тогл ячейки в строке под курсором (клавиша снимает фокус с INPUT-контролов сайдбара — навигация стрелками пропускает INPUT ради слайдеров), зажатая клавиша + ↓/↑ растягивает холд, в т.ч. кросс-блочный (undo-история на паузе, жест = один снэпшот; постановка холдов — `holds.ts:placeHoldSpan`, общий с мышиным drag; слушатель навигации в ChartGrid регистрируется один раз через navDataRef — пере-регистрация на каждый updateChart роняла шаг навигации посреди keydown-диспатча); две раскладки (View → Live input keys, `editorStore.liveKeyLayout`): UCS Lite = Z/Q/S/E/C (кол. 0–4) и NumPad 1/7/5/9/3 (кол. 5–9), StepMania = верхний ряд цифр 1…9,0 (кол. 0–9)

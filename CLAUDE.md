...
- **Рендер**: виртуализации нет — сетка блока рисуется одним `repeating-linear-gradient` фоном (`GridLayer.tsx`), ноты — memo-слой спрайтов на блок (`BlockLayer.tsx`). Вертикальные координаты спрайтов — `calc()` от `var(--rh)`, которую ставит обёртка блока `BlockSlot` (ChartGrid): пропы BlockLayer от scale не зависят, поэтому смена Scale/Zoom-строк НЕ пере-рендерит спрайты в React (раньше — фризы до ~100мс на тик колеса на плотных чартах), пересчёт делает браузер; слой стрелки кэпа короткого холда из-за этого рендерится всегда, «короткость» решают CSS-срезы (у длинного холда обратный inset клипует слой в ноль). BlockSlot и сегменты по ~64 ноты внутри блока (`SegmentSlot` в BlockLayer — блочной гранулярности мало для чартов из одного огромного блока) несут `content-visibility:auto` — оффскрин-ноты выпадают из пересчёта стилей/лейаута (тик scale на плотном одноблочном чарте в проде ~20мс → ~5мс); обёртки растянуты на `max(cw,rh)/2` под выступы спрайтов крайних строк (иначе paint containment их срезал бы), внутренний слой возвращает координаты блока. При playback c-v остаётся: сегмент дёшев на «въезде» в вьюпорт, proximity-margin браузера рендерит его заранее (замерено: worst кадр playback 9.4мс). Z-порядок между сегментами — DOM-порядок (сегменты отсортированы по строкам, что совпадает с «лесенкой» zIndex). Тики Ctrl+колеса и слайдера Scale коалесцируются в один `setTabScale` на кадр (rAF). Подписки на сторы у App/MenuBar/ChartEditor/useHitSounds — узкие (примитивные селекторы): полная подписка корня ре-рендерила бы всё дерево на каждый тик слайдера/скролла. Разделители блоков не занимают высоту (`BLOCK_DIVIDER_HEIGHT = 0`) — иначе рвались бы координаты скролла. Тело холда начинается от нижней грани клетки головы; в самой клетке — спрайт-заглушка `<dir>-Hold-HeadStub.png` (тело, обрезанное по нижнему контуру стрелки, генератор `scripts/gen-hold-head-stubs.mjs`), чтобы рельсы выходили из-под стрелки, а не висели срезом сбоку от неё. Спрайты нот укладываются «лесенкой» zIndex=1+row внутри блока (zIndex:0 на корне блока — stacking context); кэп холда получает z головы и стоит в DOM раньше неё — голова перекрывает тело и кэп. У короткого холда (клетки кэпа и головы перекрываются) кэп двухслойный: полный арт срезан clip-path по линии начала тела (иначе впечатанные в него рельсы торчали бы сбоку/выше стрелки головы), а срезанную зону дорисовывает стрелка кэпа без рельс `<dir>-Hold-BottomCapArrow.png` (рельсы срезаны по силуэту тапа) с ОБРАТНЫМ inset — слои взаимоисключающие, иначе полупрозрачный арт двоился бы и граница среза читалась контуром; стрелка кэпа цела, рельсы позади неё. Спрайты скина предзагружаются и декодируются на старте (`utils/spritePreload.ts` из `main.tsx`) — на деплое без этого ноты «прогружались на ходу». Ховер-подсветка ячейки — на собственном слое (`willChange:transform`, позиция transform'ом) и при playback не обновляется: иначе каждый mousemove пере-растеризовывал тайлы слоя нот (на плотных чартах при малом scale — просадка со 120 до ~70 FPS)
...
- **Операции над выделением** (`services/selectionOps.ts` — чистые функции; обёртки над стором — `services/editActions.ts`, их зовут и шорткаты ChartEditor, и кнопки раздела Edit в сайдбаре): Delete, Ctrl+C/X/V (внутренний клипборд; `editorStore.clipVersion` — реактивность кнопки Paste; copy/cut снимают выделение — иначе оно молча жило за экраном и перехватывало якорь вставки, вставка шла «неизвестно куда»; якорь Ctrl+V: начало выделения, без него — строка под плейхедом), Ctrl+Shift+V (вставка со сдвигом колонок), X/Y/M (flip h/v/mirror). Колесо во время Shift+drag скроллит поле и дотягивает выделение за вьюпорт (`useEditor.refreshSelectionDrag` из onScroll)
...
- **Клавиши**: Space = play/pause, Ctrl+S = сохранить, Ctrl+Z/Y = undo/redo, Ctrl+N/O/W = таб-операции, Ctrl+Tab = перебор табов, ↑↓/PgUp/PgDn/Home/End = навигация, Ctrl+колесо = per-tab Scale (зум поля — только слайдером). Полная справка: `docs/KEYBOARD.md` и в приложении (File → Keyboard shortcuts, `ShortcutsModal.tsx`). Глобальные шорткаты — в `ChartEditor.tsx`, навигация — в `ChartGrid.tsx`; текстовые поля различаются через `utils/dom.ts:isTextEntry` (пробел на слайдерах/чекбоксах уходит в play/pause)
- **Сайдбар** (слева, бывший тулбар): play, раздел Edit (иконки undo/redo, select block, delete, copy/cut/paste, flip h/v/mirror — те же `editActions`, что у шорткатов; lucide-react), per-tab Scale, Zoom поля (50–300%), Rush 0.2–2×, Volume, счётчик нот (`utils/noteCount.ts`), чекбоксы Rhythm coloring / Hit sounds / Metronome, кнопка аудио-файла. Метаданные чарта (Title/Artist/Level/Mode) — в модалке File → Chart info (`ChartInfoModal.tsx`)
...
│   ├── selectionOps.ts       — delete/copy/cut/paste/flip выделения, клипборд (чистые)
│   ├── editActions.ts        — обёртки над стором: undo/redo + операции выделения (шорткаты и сайдбар)
...
│   ├── shortcuts.ts          — isMac + shortcutLabel: подписи сочетаний ('Ctrl+Shift+V' → '⇧⌘V' на macOS)
├── components/
│   ├── editor/
